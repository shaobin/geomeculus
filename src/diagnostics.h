...
#define log_tm_print(silent, format, args...) \
    do                                        \
    {                                         \
        if (!silent)                          \
        {                                     \
...
#define log_dbg_print(silent, format, args...)                               \
    do                                                                       \
    {                                                                        \
        if (!silent)                                                         \
        {                                                                    \
            char time_s[32];                                                 \
            current_time_str(time_s, 32);                                    \
            printf("[%s] %s %s %d: ", time_s, __FILE__, __func__, __LINE__); \
            printf(format, ##args);                                          \
        }                                                                    \
    } while (0)

#define log_tm_println(silent, format, args...) \
    do                                          \
    {                                           \
        if (!silent)                            \
        {                                       \
...
#define log_dbg_println(silent, format, args...)                             \
    do                                                                       \
    {                                                                        \
        if (!silent)                                                         \
        {                                                                    \
...
#define console_printf(silent, format, args...) \
    do                                          \
    {                                           \
        if (!silent)                            \
        {                                       \
