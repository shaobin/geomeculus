...
void print_time_diff(struct timespec* t1, struct timespec* t2, char const* msg)
{
    f64 time_elapsed = diff_time_ms(t1, t2);

    if (time_elapsed < 1e3)
    {
        printf("%s: %.3f ms\n", msg, time_elapsed);
    }
    else if (time_elapsed < 6e4) // < 1 min
    {
        printf("%s: %.3f s\n", msg, time_elapsed * 1e-3);
    }
    else if (time_elapsed < 3.6e6) // < 1 h
    {
        printf("%s: %.3f min\n", msg, time_elapsed * 1e-3 / 60);
    }
    else
    {
        printf("%s: %.3f h\n", msg, time_elapsed * 1e-3 / 3600);
    }
}
